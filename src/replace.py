...
if len(sys.argv) > 1 and sys.argv[1] == "all":
		...
		if file != ".DS_Store" and (file.endswith(".png") or file.endswith(".PNG") or file.endswith(".icns") or file.endswith(".ICNS")):
			...
			if len(path_index) != 0:
				for index in path_index:
					command = './fileicon set ' + paths[index].replace(' ', '\ ') + ' ' + (icons_file + file).replace(' ', '\ ')
					# print(command)
					# os.system(command)
					# success += 1
	...
	if len(path_index) != 0:
		for index in path_index:
			# print(os.getcwd(), end="")
			command = ' ./fileicon set ' + paths[index].replace(' ', '\ ') + ' ' + (icons_file + file).replace(' ', '\ ')
			# pipe = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True, executable="/bin/zsh",).stdout
			return_info = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE,executable="/bin/zsh", stderr=subprocess.STDOUT)
